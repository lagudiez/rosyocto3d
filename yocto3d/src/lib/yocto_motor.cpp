/*********************************************************************
 *
 * $Id: yocto_motor.cpp 16185 2014-05-12 16:00:20Z seb $
 *
...
const double YMotor::BREAKINGFORCE_INVALID = YAPI_INVALID_DOUBLE;
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YMotor::set_breakingForce(double newval)
{
    string rest_val;
    char buf[32]; sprintf(buf,"%d", (int)floor(newval*65536.0 +0.5)); rest_val = string(buf);
 ...
 */
double YMotor::get_breakingForce(void)
{
    if (_cacheExpiration <= YAPI::GetTickCount()) {
        if (this->load(YAPI::DefaultCacheValidity) != YAPI_SUCCESS) {
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YMotor::set_frequency(int newval)
{
    string rest_val;
    char buf[32]; sprintf(buf, "%d", newval); rest_val = string(buf);
...
{
    return this->set_command(YapiWrapper::ysprintf("P%d,%d",(int) (targetPower*10 < 0.0 ? ceil(targetPower*10-0.5) : floor(targetPower*10+0.5)),delay));
}
 ...
 * On failure, throws an exception or returns a negative error code.
 */
int YMotor::breakingForceMove(double targetPower,int delay)
{
    return this->set_command(YapiWrapper::ysprintf("B%d,%d",(int) (targetPower*10 < 0.0 ? ceil(targetPower*10-0.5) : floor(targetPower*10+0.5)),delay));
}
